...
public class KaartjesAutomaat {
    ...
    private void voegBetaalKnoppenToe() {
        ...
        for (int i = 0; i < nKnoppen; i++) {
            ...
            Knop kg = new Knop(app, ((app.width / 2) - knoppenBreedteTotaal / 2) + (i * (knopBreedte)), app.height / 4, knopBreedte, 50, formatBedrag, 10);
            knoppenGeld.add(kg);
        }
    }
    ...
    public void voegTekenKaartjeKnopToe() {
        ...
        this.kaartjeKnop = new Knop(app, knopX, knopY, knopBreedte, knopHoogte, "Print kaartje.", 26);
    }

    public void voegTekenTerugKnopToe() {
        float knopX = (app.width / 2) + (app.width / 4);
        float knopY = (app.height / 2) + (app.height / 3);
        this.terugKnop = new Knop(app, knopX, knopY, app.width / 6, app.height / 9, "Terug", 15);
    }
    ...
    public void voegFilmsToe(String naam, double prijs) {
        ...
        Knop p = new Knop(app, app.width / 4, 10 + (knoppenFilms.size() * 120), app.width / 2, knopHoogte, f.getNaam(), 32);
        knoppenFilms.add(p);
    }
    ...
    public void voegScrollKnoppenFilmsToe() {
        ...
        String[] scrollString = {"▲", "▼"};
        for (int i = 0; i < 2; i++) {
            Knop k = new Knop(app, knopX, knopY + (i * knopHoogte * 2), knopBreedte, knopHoogte, scrollString[i], 16);
    ...
    public void drukGeldKnop() {
        for (Knop k : knoppenGeld) {
            if (app.mousePressed && k.isMuisOverKnop() && !mouseLock && !tekenDrukKnopKaartje) {
                int i = knoppenGeld.indexOf(k);
                werpGeldIn(geldBedragen[i]);
                setMouseLock(true);
            }
        }
    }
    ...
    public void drukScrollKnoppenFilms() {
        ...
        for (Knop k : scrollKnoppenFilms) {
            Knop up = scrollKnoppenFilms.get(0);
            Knop down = scrollKnoppenFilms.get(1);
            if (k.isMuisOverKnop() && app.mousePressed && !mouseLock && scrollLock > 0 && k == up) {
                for (Knop filmKnoppen : knoppenFilms) {
                    this.mouseLock = true;
                    filmKnoppen.scroll(-scrollSize);
                    this.scrollLock -= scrollSize / knoppenFilms.size();
                }
            } else if (k.isMuisOverKnop() && app.mousePressed && !mouseLock && scrollLock < scrollMax && k == down) {
                for (Knop filmKnoppen : knoppenFilms) {
                    this.mouseLock = true;
                    filmKnoppen.scroll(scrollSize);
                    this.scrollLock += scrollSize / knoppenFilms.size();
